...
if [ $# -ne $EXPECTED_ARGS ]
then
        echo "Usage: sh "$PIPELINE_TITLE"v"$PIPELINE_VERSION".sh <THIS_SAMPLE_NAME> <THIS_ANALYSIS_DIR> <START_AT_STAGE> <END_AT_STAGE> <SUBMIT_LOG>
...
## global pipeline variables ##
        ...
        COMMENTS="PIPELINE RUN ON HDC EUREKA.
...
# 1 FASTQ_MCF
        ...
        CONTAMINANTS_FASTA="$HOME"/references/Contaminants/contaminants.fa
...
# 4 MAPPING
        ...
        ALIGNER_INDEX="$HOME"/references/GRCh38_bwa/gencode.v33.basic # HUMAN ONLY
        # ALIGNER_INDEX=$HOME/references/GRCh38/Gencode/BWA/XXXX # HUMAN + DM
        # ^ this index includes the Drosophila spike-in
...
# 4) MAPPING
        ...
        then
                ...
                #Usage: 4_BWA-MEM_ALIGN.sh <SEQ_TYPE> <SAMPLE_DIR> <SAMPLE_NAME> <FASTQR1_FILE> <FASTQR2_FILE> <ALIGNER_INDEX> <BAM_OUT_FILENAME> <OUT_DIR_NAME> <THREADS>
                ...
                sbatch -W \
                        ...
                        --partition=c2s30 \
                        --wrap="\
                                sh $PIPELINE_SCRIPTS_DIR/4_"$ALIGNER".sh "$SEQ_TYPE" "$THIS_ANALYSIS_DIR"/Sample_"$THIS_SAMPLE_NAME"/ "$THIS_SAMPLE_NAME" "$THIS_ANALYSIS_DIR"/Sample_"$THIS_SAMPLE_NAME"/Processed/trimmed_"$(basename "$FASTQR1_FILE")" "$THIS_ANALYSIS_DIR"/Sample_"$THIS_SAMPLE_NAME"/Processed/trimmed_"$(basename "$FASTQR2_FILE")" "$ALIGNER_INDEX" "$THIS_SAMPLE_NAME".mapped.no-rgid.bam Alignments 25\
                                "
