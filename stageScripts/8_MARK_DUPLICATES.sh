...
if [ $# -ne $EXPECTED_ARGS ]
then
        echo "Usage: "$SCRIPT_TITLE" <SAMPLE_DIR> <ALIGNMENT_DIRNAME> <BAM_IN_FILENAME> <BAM_OUT_FILENAME> <PICARD_MEM> <mark/remove duplicates>
...
${blue}"$SCRIPT_TITLE" STARTED AT: " `date` "[JOB_ID:" $SLURM_JOB_ID" NODE_NAME:" $SLURMD_NODENAME"]" && echo -e "${NC}"
	...
	java -XX:ParallelGCThreads=5 -Xmx"$PICARD_MEM" -jar $PICARD.jar MarkDuplicates \
		INPUT="$BAM_IN_FILE" \
		OUTPUT="$BAM_OUT_FILE" \
