...
"	# Could simplify QC output by removing individual sample dirs
...
# IF statements to check/create directories
	...
	then
		echo "Making directory: "$QC_DIR_NAME"/"$OUT_DIR_NAME""
...
# cd "$QC_DIR_NAME"/"$OUT_DIR_NAME" # Simplified QC output by removing individual sample dirs # NOT SURE WHY CHANGING TO THIS DIR
 # + prevents Apptaner auto mounting Analysis dir as PWD
