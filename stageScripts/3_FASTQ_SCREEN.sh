...
FASTQ_SCREEN_CONF=$HOME/PipeLinesScripts/misc/fastq_screen.conf
ALIGNER=bowtie2
...
--subset
--outdir
--paired	# does not work in this version!
"
...
if [ $# -ne $EXPECTED_ARGS ]
then
        echo -e "Usage: "$SCRIPT_TITLE" <SEQ_TYPE> <SAMPLE_DIR> <SAMPLE_NAME> <FASTQR1_FILE> <FASTQR2_FILE> <QC_DIR_NAME> <THREADS>
...
${blue}"$SCRIPT_TITLE" STARTED AT: " `date` "[JOB_ID:" $SLURM_JOB_ID" NODE_NAME:" $SLURMD_NODENAME"]" && echo -e "${NC}"
	...
	if [ "$SEQ_TYPE" = "SE" ]
		then
			...
			srun fastq_screen --conf $FASTQ_SCREEN_CONF --aligner $ALIGNER --threads $THREADS --subset 1000000 --outdir "$QC_DIR_NAME"/fastq_screen/"$SAMPLE_NAME" "$FASTQR1_FILE"
		...
		then
			...
			srun fastq_screen --conf $FASTQ_SCREEN_CONF --aligner $ALIGNER --threads $THREADS --subset 1000000 --outdir "$QC_DIR_NAME"/fastq_screen/"$SAMPLE_NAME" --paired "$FASTQR1_FILE" "$FASTQR2_FILE"
