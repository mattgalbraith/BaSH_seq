...
do
    if [[ ! -f $WD/QC/SE_metrics/"$i"_SE_alignmentMetricsSummary.txt ]] ; then
    echo 'SE_alignmentMetricsSummary.txt does not exist. You will not have any custom plots or tables.'
    #exit 1
    fi

    counts[0]=$i
    #read the stats file 
    counts[1]=$(cat $WD/QC/SE_metrics/"$i"_SE_alignmentMetricsSummary.txt | grep 'Total Sequences' | head -1 | awk '{print $4}');
    counts[2]=$(cat $WD/QC/SE_metrics/"$i"_SE_alignmentMetricsSummary.txt | grep 'Total Sequences' | tail -1 | awk '{print $4}');
    #halve the count
    counts[3]=$(($(cat $WD/QC/SE_metrics/"$i"_SE_alignmentMetricsSummary.txt | grep "Aligned" | head -1 | awk '{print $2}') / 2));
    #halve the count
    counts[4]=$(($(cat $WD/QC/SE_metrics/"$i"_SE_alignmentMetricsSummary.txt | grep "Aligned" | tail -1 | awk '{print $2}') / 2));
    #halve the count
    counts[5]=$(($(cat $WD/QC/SE_metrics/"$i"_SE_alignmentMetricsSummary.txt | grep "Duplicates" | tail -1 | awk '{print $2}') / 2));
